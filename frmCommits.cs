...
{
    public partial class frmCommits : Form
    {
        public frmCommits()
        {
            InitializeComponent();
        ...
        public frmCommits(int projectId, string branch)
        {
            ...
            dataGridView1.AddIntColumn("라인수");
            dataGridView1.AddStringColumn("종류");
            dataGridView1.AddStringColumn("경로명", 500);
...
select c.id, c.commitNo, c.시각, c.메모, c.merge1, c.merge2, c.branch, a.학번, a.이름,
  n.추가된줄수, 삭제된줄수, f.경로명, f.종류, f.라인수, f.id sourceFileId
from [commit] c 
